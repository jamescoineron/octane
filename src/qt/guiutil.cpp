...
{
    // Convert octane:// to octane:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("octane://"))
    {
        uri.replace(0, 12, "octane:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Octane.lnk";
}
...
{
    return GetAutostartDir() / "octane.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Octane-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  octane-qt [" + tr("command-line options") + "]                     " + "\n";
